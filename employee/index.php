...
// Both Admins and Employees can view the dashboard
...
// Extract User info securely from Session
$user_fname = $_SESSION['fname'] ?? 'User';
$user_initial = strtoupper(substr($user_fname, 0, 1));

// 2. Fetch Dashboard Analytics from MySQLi
if (isset($db_conn)) {
    // Turn off error reporting temporarily so missing order tables don't crash the page
    ...
    // Map to our database's stock_quantity column
    $res = $db_conn->query("SELECT COUNT(*) AS c FROM products WHERE stock_quantity <= 5");
    if ($res) $stats['low_stock'] = $res->fetch_assoc()['c'];

    $res = $db_conn->query("SELECT COUNT(*) AS c FROM orders");
    if ($res) $stats['total_orders'] = $res->fetch_assoc()['c'];

    $res = $db_conn->query("SELECT COALESCE(SUM(total),0) AS c FROM orders WHERE DATE(created_at)=CURDATE()");
    if ($res) $stats['revenue_today'] = $res->fetch_assoc()['c'];

    // Revenue Data
    $res = $db_conn->query("SELECT DATE_FORMAT(created_at,'%b') AS month, COALESCE(SUM(total),0) AS revenue FROM orders WHERE created_at >= DATE_SUB(NOW(), INTERVAL 7 MONTH) GROUP BY DATE_FORMAT(created_at,'%Y-%m') ORDER BY created_at ASC");
    if ($res) { while ($row = $res->fetch_assoc()) $revenueData[] = $row; }

    // Recent Orders (Join with our specific 'users' table structure)
    $res = $db_conn->query("SELECT o.id, u.email AS username, CONCAT(u.fname, ' ', u.lname) AS full_name, o.total, o.status, o.created_at FROM orders o JOIN users u ON u.id=o.user_id ORDER BY o.created_at DESC LIMIT 10");
    if ($res) { while ($row = $res->fetch_assoc()) $recentOrders[] = $row; }

    // Top Products
    $res = $db_conn->query("SELECT p.name, SUM(oi.quantity) AS units, SUM(oi.quantity*oi.price) AS revenue FROM order_items oi JOIN products p ON p.id=oi.product_id GROUP BY p.id ORDER BY revenue DESC LIMIT 5");
    if ($res) { while ($row = $res->fetch_assoc()) $topProducts[] = $row; }
}

// 3. Mock Data
if (empty($recentOrders)) {
    $stats['total_products'] = $stats['total_products'] ?: 8;
    $stats['low_stock']      = $stats['low_stock'] ?: 1;
    $stats['total_orders']   = 5;
    $stats['revenue_today']  = 0;

    $recentOrders = [
        ['id'=>1,'username'=>'johndoe@example.com','full_name'=>'John Doe','total'=>1348.00,'status'=>'delivered','created_at'=>'2025-03-28 14:22:00'],
        ['id'=>2,'username'=>'johndoe@example.com','full_name'=>'John Doe','total'=>599.00, 'status'=>'shipped',  'created_at'=>'2025-03-27 09:11:00'],
        ['id'=>3,'username'=>'janedoe@example.com','full_name'=>'Jane Doe','total'=>2498.00,'status'=>'processing','created_at'=>'2025-03-26 17:44:00'],
        ['id'=>4,'username'=>'janedoe@example.com','full_name'=>'Jane Doe','total'=>299.00, 'status'=>'pending',  'created_at'=>'2025-03-25 11:00:00'],
        ['id'=>5,'username'=>'johndoe@example.com','full_name'=>'John Doe','total'=>89.00,  'status'=>'delivered','created_at'=>'2025-03-24 08:33:00'],
    ];
    $topProducts = [
...
<div class="dash-wrapper">
    ...
    <aside class="sidebar">
        ...
        <div class="sidebar-footer">
            <a href="/logout.php" class="sidebar-user">
                <div class="sidebar-avatar"><?= h($user_initial) ?></div>
                <div class="sidebar-user-info">
                    <div class="name"><?= h($user_fname) ?></div>
    ...
    <div class="dash-main">
        ...
        <div class="dash-content">
            ...
            <!-- Welcome -->
            <div class="mb-4">
                <h2 class="fw-bold mb-1">Welcome back, <?= h($user_fname) ?> 👋</h2>
