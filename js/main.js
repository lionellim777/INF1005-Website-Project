...
if (productModal) {
    productModal.addEventListener("shown.bs.modal",function(event){
        ...
        const canPurchase = button.dataset.canPurchase === '1';
        const purchaseBtn = document.getElementById("purchaseBtn");
        if (purchaseBtn) {
            if (canPurchase) {
                purchaseBtn.disabled = false;
                purchaseBtn.classList.remove('btn-secondary');
                purchaseBtn.classList.add('btn-dark');
                purchaseBtn.innerHTML = '<i class="bi bi-bag me-1"></i> Add to Cart';
                purchaseBtn.title = '';
            } else {
                purchaseBtn.disabled = true;
                purchaseBtn.classList.remove('btn-dark');
                purchaseBtn.classList.add('btn-secondary');
                purchaseBtn.innerHTML = '<i class="bi bi-ban me-1"></i> Purchase Disabled';
                purchaseBtn.title = 'Employee accounts cannot purchase items';
            }
        }
...
if (grid && countEl && sortSelect && filterBtns.length) {
    filterSort();
